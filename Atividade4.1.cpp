...
int angulo = 0;
float s = 0.0; // armazenar seno (da matriz de rotacao)
float c = 1.0;
...
void keybord_test(GLubyte key, GLint x, GLint y)
{
  ...
  case 32:
    ...
    angulo = 0;
    s = 0.0; // armazenar seno (da matriz de rotacao)
    c = 1.0;
...
void keybord_test2(GLint key, GLint x, GLint y)
{
  ...
  if (key == 100)
  {
    t_x_inc -= 1*vel; // tecla para esquerda
  }
  if (key == 102)
  {
    t_x_inc += 1*vel; // tecla para direita
  }
  if (key == 103)
  {
    t_y_inc -= 1*vel; // tecla para baixo
  }
  if (key == 101)
  {
    t_y_inc += 1*vel; // tecla para cima
  }
  if (key == 112)
  {
